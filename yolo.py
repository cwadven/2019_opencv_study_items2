...
img = cv2.imread("yolo_01.png") #이미지 읽고
img = cv2.resize(img, None, fx=0.4, fy=0.4) #사이즈를 0.4배율로
...
for i in range(len(boxes)): #전체 박스 숫자만큼 루프를 돌려서
    if i in indexes: #겹치는 박스들에서 거른 indexes만 나오게 한 박스만 보이게 만들기 위해서!
        ...
        label = str(classes[class_ids[i]]) #그 번호를 가져오면 그번호에 맞는 classes를 지정한 이름이 나온다!
        color = colors[i]
